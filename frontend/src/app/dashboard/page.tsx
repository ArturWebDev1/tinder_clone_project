...
// Define a type for a user object to improve type safety
interface User {
  id: number;
  name: string;
  age: number;
...
// Mock user data for a temporary showcase
const users: User[] = [
...
/**
 * Renders the main swiping dashboard page.
 * @param {object} props - The component props.
 * @param {(page: string) => void} props.onNavigate - Function to change the current page.
 * @param {string} props.currentPage - The name of the currently active page.
 */
export default function DashboardPage({ onNavigate, currentPage }: { onNavigate: (page: string) => void, currentPage: string }) {
  const [cardStack, setCardStack] = useState<User[]>(() => users.slice(0, 2));
  const [isSwiping, setIsSwiping] = useState<boolean>(false);
  const [translation, setTranslation] = useState<{ x: number; rotation: number }>({ x: 0, rotation: 0 });
  const [swipeDirection, setSwipeDirection] = useState<'left' | 'right' | null>(null);

  // localStorage.setItem("currentPage", currentPage);

  // useRef to hold the top card element for DOM manipulation
  const cardRef = useRef<HTMLDivElement | null>(null);
  // useRef to store the starting x coordinate of the swipe
  ...
  useEffect(() => {
    ...
    // Type-safe event listeners for mouse and touch events
    ...
    card.addEventListener('mousedown', onMouseDown as any);
    window.addEventListener('mousemove', onMouseMove as any);
    window.addEventListener('mouseup', onMouseUp);
    card.addEventListener('touchstart', onTouchStart as any);
    card.addEventListener('touchmove', onTouchMove as any);
    window.addEventListener('touchend', onTouchEnd);

    return () => {
      card.removeEventListener('mousedown', onMouseDown as any);
      window.removeEventListener('mousemove', onMouseMove as any);
      window.removeEventListener('mouseup', onMouseUp);
      card.removeEventListener('touchstart', onTouchStart as any);
      card.removeEventListener('touchmove', onTouchMove as any);
  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center bg-white font-inter overflow-x-hidden">
      ...
      <div className="absolute bottom-[80px] left-0 right-0 flex justify-center items-center gap-4 z-10">
        ...
        <button onClick={() => handleSwipe('left')} className={`h-16 w-16 flex items-center justify-center rounded-full border border-red-500 transition-all duration-300 ${swipeDirection === 'left' ? 'bg-red-500' : 'bg-transparent'}`}>
        ...
        <button onClick={() => handleSwipe('right')} className={`h-16 w-16 flex items-center justify-center rounded-full border border-green-500 transition-all duration-300 ${swipeDirection === 'right' ? 'bg-green-500' : 'bg-transparent'}`}>
